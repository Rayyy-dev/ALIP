...
class LogService {
  ...
  async ingestLog(input: LogInput) {
    ...
    // Directly await trend update and alert evaluation (serverless-compatible)
    try {
      await trendService.updateTrendsForLog(validated.service, validated.level);
    } catch (err) {
      logger.error('Failed to update trends', { error: (err as Error).message });
    }
    try {
      await alertService.evaluateAlerts(validated.service);
    } catch (err) {
  ...
  async ingestBatch(input: BatchLogInput) {
    ...
    for (let i = 0; i < validated.logs.length; i += chunkSize) {
      ...
      const results = await Promise.allSettled(
        chunk.map((log) => this.ingestLog(log as unknown as LogInput))
      );
